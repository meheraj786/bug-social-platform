  ...
  const acceptFriendReq = (user) => {
    ...
    toast.custom((t) => (
      <CustomToast
        ...
        content={`You accept Friend Request from ${user.sendername}`}
      />
    ));
    const dontShow = true;
...
return (
  <div className="max-w-[900px] mt-10 mx-auto">
    <motion.div
      ...
      className="w-full font-secondary bg-white/90 backdrop-blur-xl shadow-2xl rounded-2xl border border-gray-200/50 p-6 space-y-5"
    >
      ...
      {friendRequestList.length > 0 && (
        <div className="space-y-4">
          ...
          {/* Horizontal Scrollable Container for Friend Requests */}
          <div className="overflow-x-auto pb-4">
            <div className="flex gap-4 min-w-max">
              {friendRequestList.map((user, idx) => (
                <div
                  key={idx}
                  className="flex-shrink-0 w-72 p-4 bg-gradient-to-r from-purple-50/80 to-blue-50/80 rounded-2xl border border-purple-200/50 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 group"
                >
                  ...
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="text-xs bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 text-white px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 font-medium flex items-center gap-1.5 flex-1 justify-center"
                    >
                      <svg
                        className="w-3 h-3"
                        fill="currentColor"
                    ...
                    <button
                      ...
                      className="text-xs bg-white hover:bg-red-50 border border-gray-300 hover:border-red-300 text-gray-600 hover:text-red-600 px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 font-medium flex items-center gap-1.5 flex-1 justify-center"
                    >
                      <svg
                        className="w-3 h-3"
                        fill="none"
      ...
      <div className="space-y-4">
        ...
        {/* Horizontal Scrollable Container for Suggested Users */}
        <div className="overflow-x-auto pb-4">
          <div className="flex gap-4 min-w-max">
            {userList
              .filter(
              ...
              .map((user, idx) => (
                <div
                  key={idx}
                  className="flex-shrink-0 w-72 p-4 bg-gradient-to-r from-gray-50/80 to-white/80 rounded-2xl border border-gray-200/50 hover:shadow-lg hover:scale-[1.02] transition-all duration-300 group"
                >
                  <div className="flex items-center gap-4 mb-4">
                    <div className="relative">
                      <img
                  ...
                  <div className="flex items-center">
                    ...
                    requestList.includes(currentUser.uid + user.id) ? (
                      <button
                        ...
                        className="text-xs bg-gradient-to-r from-orange-500 to-red-500 hover:from-orange-600 hover:to-red-600 text-white px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 font-medium flex items-center gap-1.5 w-full justify-center"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
                      ...
                      <button
                        ...
                        className="text-xs bg-gradient-to-r from-blue-500 to-purple-500 hover:from-blue-600 hover:to-purple-600 text-white px-4 py-2 rounded-full shadow-md hover:shadow-lg hover:scale-105 active:scale-95 transition-all duration-200 font-medium flex items-center gap-1.5 w-full justify-center"
                      >
                        <svg
                          className="w-3 h-3"
                          fill="none"
