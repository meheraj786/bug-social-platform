...
import TypingAnimation from "../../layouts/TypingAnimation";
...
const typingTracker = () => {
  const id =
    roomuser.senderid == data?.uid ? roomuser.reciverid : roomuser.senderid;
  const typeRef = push(ref(db, "type"));
  ...
  set(typeRef, {
    ...
    const unsubscribe = onValue(requestRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        const type = item.val();
        if (type.senderid == reciverId && type.reciverid==data?.uid) {
          arr.push({ ...type, id: item.key })
        }
      });
      setTyping(arr);
    });
    ...
    <>
      {roomuser.pageName ? (
        ...
        >
          ...
          <div className="flex-shrink-0 relative">
            {
              typing.length>0 && <span className="absolute -top-10 left-10"><TypingAnimation/></span>
            }
