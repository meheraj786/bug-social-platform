    ...
    useEffect(() => {
      ...
      onValue(followRef, (snapshot) => {
        snapshot.forEach((data) => {
          ...
          if (follow.followerid==chatId && follow.followingid==page) {
            setRoomuser({ ...follow, id: data.key });
          }
        });
      });
    }, [db, chatId, page]);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(messageRef, (snapshot) => {
      ...
      snapshot.forEach((item) => {
        ...
        if (
          (message.senderid === chatId && message.reciverid === page) ||
          (message.senderid === page && message.reciverid === chatId)
        ) {
    ...
    return () => unsubscribe();
  }, [db, chatId, page, roomuser]);

  const sentMessageHandler = (like) => {
    const senderid = roomuser.followingid;
    const sendername = roomuser.followingname;

    let reciverid, recivername;
      reciverid = roomuser.followerid;
      recivername = roomuser.followername;
      ...
      >
        ...
        >
          <div className="relative">
            <img
              src={
                roomuser.followerimg
              }
              alt={roomuser.followername}
              className="w-12 h-12 rounded-full object-cover ring-2 ring-white"
            />
            {/* <div className="absolute -bottom-1 -right-1 w-4 h-4 bg-green-500 rounded-full border-2 border-white"></div> */}
          </div>
          <span className="font-medium text-gray-800 truncate flex-1">
            {roomuser.followername}
          </span>
          ...
          {messageList?.map((msg) => (
            ...
            >
              ...
              border border-white/30 text-gray-700 text-xs rounded-full shadow-lg`}
                ...
                {msg.msgImg && msg.status != "deleted" && (
                  <div
                    className={`flex items-end gap-3 ${
                      msg.senderid === roomuser.followingid
                        ? "justify-end"
                        : "justify-start"
                    }`}
                  >
                ...
                <div
                  className={`flex items-end gap-3 ${
                    msg.senderid === roomuser.followingid ? "justify-end" : "justify-start"
                  }`}
                >
                  {/* Delete Button */}
                  {msg.senderid === roomuser.followingid && msg.status != "deleted" && (
                    <button
                      onClick={() => {
                  ...
                  {msg.message && (
                    <div
                      className={`relative max-w-[70%] ${
                        msg.senderid === roomuser.followingid ? "order-1" : "order-2"
                      }`}
                    >
                      <div
